...
ELCPP <- function(z, ct, mu, shift, lambda_init, return_weights, lower, upper, order, weight_tolerance, deriv = FALSE, thresh = 1e-16, itermax = 100L, verbose = FALSE, alpha = 0.3, beta = 0.8, backeps = 0.0, grad_tol = 1e-12, step_tol = 1e-12, f_tol = 1e-14, stallmax = 5L) {
    .Call(`_smoothemplik_ELCPP`, z, ct, mu, shift, lambda_init, return_weights, lower, upper, order, weight_tolerance, deriv, thresh, itermax, verbose, alpha, beta, backeps, grad_tol, step_tol, f_tol, stallmax)
}

EuLCPP <- function(z, mu, ct, vt, shift, n_orig, weight_tolerance, trunc_to = 0.0, renormalise = TRUE, return_weights = FALSE, verbose = FALSE) {
    .Call(`_smoothemplik_EuLCPP`, z, mu, ct, vt, shift, n_orig, weight_tolerance, trunc_to, renormalise, return_weights, verbose)
}
...
dampedNewtonCPP <- function(fn, par, thresh = 1e-16, itermax = 100L, verbose = FALSE, alpha = 0.3, beta = 0.8, backeps = 0.0, grad_tol = 1e-12, step_tol = 1e-12, f_tol = 1e-14, stallmax = 5L) {
    .Call(`_smoothemplik_dampedNewtonCPP`, fn, par, thresh, itermax, verbose, alpha, beta, backeps, grad_tol, step_tol, f_tol, stallmax)
}
